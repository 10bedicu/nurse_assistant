...
const formatMedicationDosage = (dosageInstruction: unknown) => {
  if (!Array.isArray(dosageInstruction) || dosageInstruction.length === 0) {
    return "";
  }
  ...
  return dosageInstruction
    .map((dosage: any) => {
      return (
        dosage.text ||
        `${dosage.dose_and_rate?.dose_quantity?.value ?? ""} ${dosage.dose_and_rate?.dose_quantity?.unit?.display ?? ""}`.trim() ||
        "N/A"
      );
    })
    .join("; ");
...
const formatMedicationRoutes = (dosageInstruction: unknown) => {
  if (!Array.isArray(dosageInstruction) || dosageInstruction.length === 0) {
    return "";
  }

  return dosageInstruction
    .map((dosage: any) => dosage.route?.display)
    .filter(Boolean)
    .join(", ");
};
...
export default function NurseAssistant(props: NurseAssistantProps) {
  ...
  const { patient, patientId, selectedEncounterId, facilityId, encounter } =
    props;
  ...
  const { data: allergies, isLoading: isAllergiesLoading } = useQuery({
    queryKey: ["na-allergies", patientId, selectedEncounterId],
    queryFn: ({ signal }) =>
      API.nurseAssistant.listAllergies({
        patientId: patientId ?? "",
        selectedEncounterId,
        selectedEncounterStatus: encounter?.status,
        signal,
      }),
    enabled: !!patientId,
  });
  ...
  const { data: symptoms, isLoading: isSymptomsLoading } = useQuery({
    queryKey: ["na-symptoms", patientId, selectedEncounterId],
    queryFn: ({ signal }) =>
      API.nurseAssistant.listSymptoms({
        patientId: patientId ?? "",
        selectedEncounterId,
        signal,
      }),
  ...
  const { data: diagnoses, isLoading: isDiagnosesLoading } = useQuery({
    queryKey: ["na-diagnoses", patientId, selectedEncounterId],
    queryFn: ({ signal }) =>
      API.nurseAssistant.listDiagnoses({
        patientId: patientId ?? "",
        selectedEncounterId,
        signal,
      }),
  ...
  const { data: medications, isLoading: isMedicationsLoading } = useQuery({
    queryKey: ["na-medications", patientId, selectedEncounterId, facilityId],
    queryFn: ({ signal }) =>
      API.nurseAssistant.listMedications({
        patientId: patientId ?? "",
        selectedEncounterId,
        facilityId,
        signal,
  ...
  const { data: medicationStatement, isLoading: isMedicationStatementLoading } =
    useQuery({
      queryKey: ["na-medication-statements", patientId],
      queryFn: ({ signal }) =>
        API.nurseAssistant.listMedicationStatements({
          patientId: patientId ?? "",
          signal,
        }),
      enabled: !!patientId,
  ...
  const { data: serviceRequests, isLoading: isServiceRequestsLoading } =
    useQuery({
      ...
      queryFn: ({ signal }) =>
        API.nurseAssistant.listServiceRequests({
          selectedEncounterId,
...
${formatMedicationDosage(med.dosage_instruction) ? `Dosage: ${formatMedicationDosage(med.dosage_instruction)}` : ""}
${
  formatMedicationRoutes(med.dosage_instruction)
    ? `Route: ${formatMedicationRoutes(med.dosage_instruction) || "N/A"}`
    : ""
}
...
`
  ...
  const setupSessionListeners = useCallback((session: RealtimeSession) => {
    ...
    // Transcript done
    session.transport.on("audio_transcript_done", () => {
      streamingMessageRef.current = null;
    });
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "nurse-assistant-container invisible fixed right-4 bottom-0 z-10 flex translate-y-10 scale-90 items-end p-6 opacity-0 transition-all duration-300",
        show && "visible translate-y-0 scale-100 opacity-100",
      )}
    >
      {/* Persona avatar */}
      ...
      <div className="relative p-4">
        <div className="absolute inset-0 -z-10 mask-[radial-gradient(ellipse_60%_60%_at_center,black_60%,transparent_100%)] backdrop-blur-sm" />
        ...
        {/* Messages */}
        <div className="max-h-[70vh] w-80 overflow-y-auto text-sm">
          {messages.length === 0 && !isConnected ? (
            <div className="mb-2 max-w-[90%] rounded-lg bg-neutral-200/50 p-4 backdrop-blur-sm">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1">
                        {referencedContexts.map((context) => (
                          <a
                            ...
                            className="inline-flex items-center gap-1 rounded-md bg-black/10 px-1.5 py-0.5 text-[10px] underline transition-colors hover:bg-black/20"
                          >
                            📄 {context.name}
                          </a>
                        ))}
        ...
        <div className="relative">
          <textarea
            ...
            className="m-0 min-h-20 w-full min-w-60 resize-none rounded-lg border-black/10 bg-white/50 p-2 backdrop-blur-sm disabled:opacity-50"
          />
          <div className="flex items-center justify-between gap-2 p-1">
            ...
            <div className="flex items-center gap-1">
              <select
                ...
                className={cn(
                  "w-20 rounded-md border-black/10 bg-white/50 px-2 py-1 text-xs backdrop-blur-sm",
              ...
              {(isConnected || messages.length > 0) && (
                <button
                  onClick={newChat}
                  className="flex aspect-square w-8 items-center justify-center rounded-full bg-neutral-200 transition-colors hover:bg-neutral-300"
                  title="New chat"
                >
            ...
            <div className="flex items-center gap-2">
              ...
              {messages.length > 0 && (
                ...
                >
                  ...
                  <span
                    className={cn(isLimitReached && "font-medium text-red-500")}
                  >
